...
class SignupRequest(BaseModel):
    ...
    mail_provider: str = Field(
        default="worker",
        description="Provider: 'worker', 'icloud_api', 'outlook', 'dongvanfb', hoặc 'gmail_advanced'.",
        pattern="^(worker|icloud_api|outlook|dongvanfb|gmail_advanced)$",
    )
    ...
    # iCloud rental inbox API config
    icloud_api_inbox_url: str = Field(
        default="https://mail.congas.uk/api/rentals/inbox",
        description="iCloud rental inbox API URL cho mail_provider='icloud_api'.",
    )
    icloud_api_rental_token: str | None = Field(
        default=None,
        description="Rental token cho iCloud API OTP.",
    )
