...
_EXACT_KEYS: frozenset[str] = frozenset([
    ...
    "reg.post_reg_check_plan", "reg.post_reg_link_region", "reg.auto_retry", "reg.auto_retry_max",
    ...
    "session.login_flow", "session.max_concurrent", "session.job_timeout",
    "session.reuse_enabled", "session.revalidate_http", "session.cookie_max_age_hours",
    "telegram.bot_token", "telegram.chat_id", "telegram.groups",
...
def _validate_type_constraint(key: str, value: Any) -> None:
    ...
    if key in ("reg.headless", "reg.debug", "reg.post_reg_get_session",
               "reg.post_reg_get_link", "reg.post_reg_check_plan",
               "reg.auto_retry", "reg.use_proxy",
               "reg.mfa_inline"):
    ...
    if key == "telegram.groups":
        if value is None:
            return
        if not isinstance(value, list):
            raise RepositoryError(
                "set", TypeError(f"{key}: must be list or null, got {type(value).__name__}")
            )
        for item in value:
            if not isinstance(item, dict):
                raise RepositoryError("set", TypeError(f"{key}: each item must be object"))
            chat_id = item.get("chat_id")
            if not isinstance(chat_id, str) or not chat_id.strip() or len(chat_id) > 64:
                raise RepositoryError("set", ValueError(f"{key}: each chat_id must be non-empty str <= 64"))
            title = item.get("title")
            if title is not None and (not isinstance(title, str) or len(title) > 120):
                raise RepositoryError("set", ValueError(f"{key}: title must be str <= 120"))
            bot_token = item.get("bot_token")
            if bot_token is not None and (not isinstance(bot_token, str) or len(bot_token) > 200):
                raise RepositoryError("set", ValueError(f"{key}: bot_token must be str <= 200"))
            enabled = item.get("enabled")
            if enabled is not None and not isinstance(enabled, bool):
                raise RepositoryError("set", TypeError(f"{key}: enabled must be bool"))
            for stat_key in ("qr_sent", "plus_count"):
                val = item.get(stat_key)
                if val is not None and (not isinstance(val, int) or isinstance(val, bool) or val < 0):
                    raise RepositoryError("set", TypeError(f"{key}: {stat_key} must be non-negative int"))
            qr_messages = item.get("qr_messages")
            if qr_messages is not None:
                if not isinstance(qr_messages, dict):
                    raise RepositoryError("set", TypeError(f"{key}: qr_messages must be object"))
                for msg_job_id, msg_id in qr_messages.items():
                    if not isinstance(msg_job_id, str) or len(msg_job_id) > 64:
                        raise RepositoryError("set", ValueError(f"{key}: qr_messages keys must be str <= 64"))
                    if not isinstance(msg_id, int) or isinstance(msg_id, bool) or msg_id <= 0:
                        raise RepositoryError("set", TypeError(f"{key}: qr_messages values must be positive int"))
        return
