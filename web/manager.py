...
class Job:
    ...
    plan: str | None = None  # post-reg entitlement plan label
    plan_check: dict[str, Any] | None = None  # post-reg raw plan check result
...
class JobManager:
    ...
    def __init__(self, *, max_concurrent: int = 1, job_repo: "JobRepository | None" = None, combo_repo: "ComboRepository | None" = None, session_repo: "SessionResultRepository | None" = None):
        ...
        self._post_reg_check_plan: bool = True
    ...
    @property
    def post_reg_check_plan(self) -> bool:
        return self._post_reg_check_plan

    def set_post_reg_check_plan(self, value: bool) -> None:
        self._post_reg_check_plan = bool(value)
    ...
    def apply_settings(self, settings: dict) -> None:
        ...
        if "reg.post_reg_check_plan" in settings:
            self._post_reg_check_plan = bool(settings["reg.post_reg_check_plan"])
    ...
    def clear_finished(self) -> int:
        """Xoá tất cả jobs đã xong (success/error) khỏi memory và SQLite.

        Cascade xoá job_logs trong SQLite tự động qua FK ON DELETE CASCADE.
    ...
    async def clear_all(self) -> int:
        ...
        Cancel running tasks trước khi xoá. Drain queue.
        """
    ...
    async def _run_job(self, job: Job) -> None:
        try:
            ...
            if self._post_reg_get_session or self._post_reg_get_link or self._post_reg_check_plan:
    ...
    async def _post_reg_steps(self, job: Job, result: SignupResult) -> None:
        """Execute enabled post-reg toggles after signup success.

        Mỗi step độc lập: lỗi 1 step không ảnh hưởng step khác và không làm fail
        overall job (reg+2FA đã thành công).
        """
        coros = []
        if self._post_reg_check_plan:
            coros.append(self._post_reg_check_plan_step(job, result.access_token, result.cookies))
    ...
    async def _post_reg_check_plan_step(
        self,
        job: Job,
        access_token: str | None,
        cookies: list | None,
    ) -> None:
        """Post-reg: check entitlement plan using the fresh access token."""
        try:
            self._job_log(job, "[post-reg] checking plan...")
            if not access_token:
            ...
            plan = data.get("plan")
            job.plan = str(plan) if plan else None
            job.plan_check = {
                "ok": True,
                "plan": job.plan,
                "is_plus": bool(data.get("is_plus")),
                "has_active_subscription": bool(data.get("has_active_subscription")),
                "expires": data.get("expires"),
            }
            badge = "PLUS" if job.plan_check["is_plus"] else (job.plan or "free").upper()
            self._job_log(job, f"[post-reg] plan checked: {badge}")
            self._broadcast_job(job)
        except Exception as exc:
            job.plan_check = {
                "ok": False,
...
class SessionJobManager:
    ...
    def __init__(self, *, max_concurrent: int = 5, job_repo: "JobRepository | None" = None):
    ...
    def apply_settings(self, settings: dict) -> None:
        ...
        if "reg.debug" in settings:
            self._debug = bool(settings["reg.debug"])
        timeout_key = "session.job_timeout" if "session.job_timeout" in settings else "reg.job_timeout"
        if timeout_key in settings:
            val = float(settings[timeout_key])
            if 30 <= val <= 600:
                self._job_timeout = val
        _hydrate_proxy_pool_from_settings(settings)
        if "session.max_concurrent" in settings:
            val = int(settings["session.max_concurrent"])
            if 1 <= val <= 10:
                self._max = val
        if "reg.auto_retry" in settings:
            self._auto_retry = bool(settings["reg.auto_retry"])
    ...
    async def _run_job(self, job: SessionJob) -> None:
        ...
        try:
            ...
            # strip __cookies trước khi persist DB jobs / broadcast SSE (R4.5).
            job.session_data = _strip_session_cookies(session_data)
            if await self._persist_status(
                job, "success",
                session_data=json.dumps(job.session_data, ensure_ascii=False) if job.session_data else None,
            ):
...
def get_session_manager(job_repo: "JobRepository | None" = None) -> SessionJobManager:
    ...
    if _session_manager is None:
        _session_manager = SessionJobManager(max_concurrent=5, job_repo=job_repo)
...
class UpiJob:
    ...
    _telegram_qr_message_id: int | None = field(default=None, repr=False)
    _telegram_plus_notified: bool = field(default=False, repr=False)
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "telegram_qr_message_id": self._telegram_qr_message_id,
            "telegram_qr_sent": bool(self._telegram_qr_message_id),
            "telegram_plus_notified": self._telegram_plus_notified,
...
class UpiJobManager:
    ...
    # ── Public CRUD ─────────────────────────────────────────────────────
    def add_jobs(self, combos: list[str]) -> list[UpiJob]:
        existing_emails = {j.email.lower() for j in self.jobs.values() if j.status != "cancelled"}
        out: list[UpiJob] = []
        for raw in combos:
            line = raw.strip()
    ...
    async def check_plan(self, job_id: str) -> dict[str, Any]:
        ...
        await self._notify_plus_if_needed(job, result)
        self._broadcast_job(job)
        return result
    ...
    async def retry_job(self, job_id: str) -> bool:
        ...
        job._telegram_chat_id = None
        job._telegram_qr_message_id = None
        job._telegram_plus_notified = False
        job.plan_check = None
        retry_line = f"[{datetime.now():%H:%M:%S}] -- retry --"
        job.log_lines.append(retry_line)
        self._broadcast_job(job)
    ...
    async def _notify_telegram(self, job: UpiJob) -> None:
        ...
        try:
            ...
            if sent:
                job._telegram_chat_id = notifier.chat_id
                if isinstance(sent, int):
                    job._telegram_qr_message_id = sent
                try:
                    from .server import _persist_telegram_groups
                    _persist_telegram_groups()
                except Exception as exc:  # noqa: BLE001
                    self._job_log(job, f"[tg]   stats           -  persist fail: {type(exc).__name__}: {exc}")
                asyncio.create_task(self._post_qr_check_plan(job.id, notifier.chat_id))
                self._broadcast_job(job)
    ...
    # ── Run job ─────────────────────────────────────────────────────────
    async def _notify_plus_if_needed(
        self,
        job: UpiJob,
        plan_result: dict[str, Any],
        telegram_chat_id: str | None = None,
    ) -> None:
        if not plan_result.get("ok") or not plan_result.get("is_plus"):
            return
        if job._telegram_plus_notified:
            return
        from .telegram_notifier import get_telegram_notifier

        notifier = get_telegram_notifier()
        target_chat_id = telegram_chat_id or job._telegram_chat_id or notifier.chat_id
        if not notifier.enabled or not target_chat_id:
            return
        reply_to = job._telegram_qr_message_id or notifier.qr_message_id_for(target_chat_id, job.id)
        notifier.mark_plus(target_chat_id)
        job._telegram_plus_notified = True
        try:
            await notifier.notify_plus_success(
                email=job.email,
                job_id=job.id,
                plan=str(plan_result.get("plan") or "plus"),
                chat_id=target_chat_id,
                reply_to_message_id=reply_to,
                log=lambda msg: self._job_log(job, msg),
            )
        except Exception as exc:  # noqa: BLE001
            self._job_log(job, f"[tg]   plus-notify     -  {type(exc).__name__}: {exc}")
        try:
            from .server import _persist_telegram_groups
            _persist_telegram_groups()
        except Exception as exc:  # noqa: BLE001
            self._job_log(job, f"[tg]   plus-stats      -  persist fail: {type(exc).__name__}: {exc}")

    async def _post_qr_check_plan(self, job_id: str, telegram_chat_id: str | None = None) -> None:
        """Check plan sau khi QR đã gửi Telegram để cập nhật thống kê group."""
        ...
        try:
            ...
            _log.warning("UpiMgr: post-QR check_plan failed for %s: %s", job_id, exc)
