...
def _parse_icloud_api_line(line: str) -> ParsedLine:
    parts = line.strip().split("|", 1)
    email = parts[0].strip()
    if not _EMAIL_RE.match(email):
        raise MailModeParseError(f"invalid icloud api email: {line[:80]}")
    return ParsedLine(email=email, raw=line)


def _build_icloud_api_request(
    parsed: ParsedLine,
    *,
    worker_config: dict[str, str] | None = None,
    password: str | None = None,
    ...
    cfg = worker_config or {}
    parts = parsed.raw.strip().split("|", 1)
    rental_token = parts[1].strip() if len(parts) == 2 else str(cfg.get("rental_token", "")).strip()
    from config import env_insecure_tls
    return SignupRequest(
        email=parsed.email,
        mail_provider="icloud_api",
        icloud_api_inbox_url=cfg.get("inbox_url", "https://mail.congas.uk/api/rentals/inbox"),
        icloud_api_rental_token=rental_token,
        otp_timeout_seconds=300.0,
        otp_poll_interval_seconds=5.0,
        otp_resend_after_seconds=120.0,
        headless=headless,
...
ICLOUD_API_MODE = MailModeSpec(
    id="icloud_api",
    label="iCloud API",
    input_placeholder="pedicab_21_wraiths@icloud.com|TDJA7XURS2H4",
    input_help="Mỗi dòng: email|rentalToken. Nếu nhập Rental Token ở cấu hình thì có thể chỉ nhập email.",
    config_schema=[
        {
            "key": "inbox_url",
            "label": "iCloud API URL",
            "type": "text",
            "default": "https://mail.congas.uk/api/rentals/inbox",
            "required": True,
            "validate_prefix": ["http://", "https://"],
        },
        {
    ...
    parse_line=_parse_icloud_api_line,
    build_request=_build_icloud_api_request,
)
...
_REGISTRY: dict[str, MailModeSpec] = {
    ...
    ICLOUD_API_MODE.id: ICLOUD_API_MODE,
