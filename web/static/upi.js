...
(() => {
  ...
  const dom = {
    ...
    telegramGroupSelect: $('upi-telegram-group-select'),
    ...
    kpiAccounts:  $('upi-kpi-accounts'),
    kpiJobs:      $('upi-kpi-jobs'),
    kpiRunning:   $('upi-kpi-running'),
    kpiWaiting:   $('upi-kpi-waiting'),
    kpiPlus:      $('upi-kpi-plus'),
    kpiProxy:     $('upi-kpi-proxy'),
    kpiTelegram:  $('upi-kpi-telegram'),
    kpiMode:      $('upi-kpi-mode'),
    kpiStatus:    $('upi-kpi-status'),
    // Modal
    modal:         $('upi-qr-modal'),
  ...
  function renderTelegramGroupSelect(groups, selectedChatId) {
    if (!dom.telegramGroupSelect) return;
    groups = Array.isArray(groups) ? groups : [];
    const enabled = groups.filter((g) => g.enabled !== false);
    const opts = ['<option value="">Chua chon group</option>'].concat(enabled.map((g) => {
      const id = String(g.chat_id || '');
      const label = `${g.title || id} (${id})`;
      return `<option value="${escHtml(id)}"${id === String(selectedChatId || '') ? ' selected' : ''}>${escHtml(label)}</option>`;
    }));
    dom.telegramGroupSelect.innerHTML = opts.join('');
  }

  function loadTelegramGroupsForUpi() {
    return api('/api/telegram/config').then((data) => {
      renderTelegramGroupSelect(data.groups || [], data.chat_id || '');
      return data;
    }).catch(() => {});
  }
  ...
  function updateComboCount() {
    ...
    if (dom.kpiAccounts) dom.kpiAccounts.textContent = String(lines.length);
  }
  ...
  function renderJobs() {
    if (state.order.length === 0) {
      ...
      updateUpiDashboardStats({ queued: 0, running: 0, success: 0, error: 0, cancelled: 0, waitingScan: 0, plus: 0 });
      return;
    }

    const stats = { queued: 0, running: 0, success: 0, error: 0, cancelled: 0, waitingScan: 0, plus: 0 };
    const html = state.order.map((id, idx) => {
      const j = state.jobs.get(id);
      if (!j) return '';
      stats[j.status] = (stats[j.status] || 0) + 1;
      const isPlus = !!(j.plan_check && j.plan_check.is_plus);
      if (j.status === 'success' && j.has_qr && !isPlus) stats.waitingScan += 1;
      if (isPlus) stats.plus += 1;
      const cls = [
        'job',
        state.activeJobId === id ? 'is-active' : '',
        j.telegram_qr_sent ? 'upi-telegram-sent' : '',
      ].filter(Boolean).join(' ');
      ...
      if (j.status === 'success' && j.has_qr) {
        ...
        if (!j.telegram_qr_sent) {
          actionBtns += `<button class="icon-btn upi-send-telegram-btn" data-action="send-telegram" data-id="${escHtml(id)}" title="Gửi QR vào Tele" aria-label="Gửi QR vào Tele"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="m22 2-7 20-4-9-9-4Z"/><path d="M22 2 11 13"/></svg></button>`;
        }
      }
        ...
        <div class="${cls}" data-id="${escHtml(id)}">
          ...
          <div class="job-main">
            <div class="job-email" title="${escHtml(j.email)}">
              ...
              ${countdownBadge}
              ${planBadge}
              ${j.telegram_qr_sent ? '<span class="badge upi-telegram-sent-badge" title="QR đã gửi vào Telegram">TELE ✓</span>' : ''}
              ${cycleBadge}
              ${errBadge}
            </div>
          </div>
    ...
    dom.jobSummary.textContent = [
      ...
      stats.waitingScan ? `cho_quet ${stats.waitingScan}` : '',
      stats.plus ? `plus ${stats.plus}` : '',
      stats.error ? `${stats.error} failed` : '',
    ].filter(Boolean).join(' · ');
    updateUpiDashboardStats(stats);
    updateCountdowns();
  }

  function updateUpiDashboardStats(stats) {
    if (dom.kpiJobs) dom.kpiJobs.textContent = String(state.order.length);
    if (dom.kpiRunning) dom.kpiRunning.textContent = String(stats.running || 0);
    if (dom.kpiWaiting) dom.kpiWaiting.textContent = String(stats.waitingScan || 0);
    if (dom.kpiPlus) dom.kpiPlus.textContent = String(stats.plus || 0);
    if (dom.kpiProxy && dom.proxyFromStep) dom.kpiProxy.textContent = String(dom.proxyFromStep.value || '-');
    if (dom.kpiTelegram && dom.notifyToggle) dom.kpiTelegram.textContent = dom.notifyToggle.checked ? 'On' : 'Off';
    if (dom.kpiMode) {
      const mode = $('mode');
      dom.kpiMode.textContent = mode && mode.selectedOptions && mode.selectedOptions[0]
        ? mode.selectedOptions[0].textContent
        : '-';
    }
    if (dom.kpiStatus) {
      const pill = $('status-pill');
      dom.kpiStatus.textContent = pill ? pill.textContent : 'Idle';
    }
  }
  ...
  dom.jobList.addEventListener('click', (e) => {
    ...
    if (actionBtn) {
      ...
      if (action === 'retry') {
        ...
        window.GptUi.toast('Đang gửi QR vào Telegram…', { type: 'info' });
        api(`/api/upi/jobs/${encodeURIComponent(id)}/notify`, { method: 'POST' })
          .then((data) => {
            if (data && data.job && data.job.id) {
              state.jobs.set(data.job.id, data.job);
            } else {
              const cur = state.jobs.get(id);
              if (cur) cur.telegram_qr_sent = true;
            }
            renderJobs();
            window.GptUi.toast('Đã gửi QR vào Telegram.', { type: 'success' });
          })
          .catch(async (err) => {
            await Dialog.alert({ message: 'Gửi Telegram thất bại: ' + err.message });
          })
  ...
  if (dom.telegramGroupSelect) {
    dom.telegramGroupSelect.addEventListener('change', async () => {
      try {
        ...
        await api('/api/telegram/config', {
          ...
          body: JSON.stringify({
            ...
            chat_id: dom.telegramGroupSelect.value || '',
          }),
        });
      } catch (err) {
        await Dialog.alert({ message: 'Khong luu duoc Telegram group: ' + err.message });
        loadTelegramGroupsForUpi();
      }
    });
  }
