...
(function () {
  ...
  function alert(opts) {
    _validate(opts);
    if (opts.modal !== true && window.GptUi && typeof window.GptUi.toast === "function") {
      const text = opts.detail ? (opts.message + " · " + opts.detail) : opts.message;
      const hay = (opts.title || "") + " " + opts.message;
      const lower = hay.toLowerCase();
      const type = opts.type || (
        lower.indexOf("fail") >= 0 ||
        lower.indexOf("error") >= 0 ||
        lower.indexOf("thất bại") >= 0 ||
        lower.indexOf("that bai") >= 0 ||
        lower.indexOf("lỗi") >= 0 ||
        lower.indexOf("loi") >= 0
          ? "error"
          : lower.indexOf("warn") >= 0 || lower.indexOf("cảnh báo") >= 0
            ? "warn"
            : "info"
      );
      window.GptUi.toast(text, { type: type, duration: type === "error" ? 3600 : 2200 });
